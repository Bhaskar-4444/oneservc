...
const category_images = {
    'paint': 'https://blog.75services.com/wp-content/uploads/2023/09/residential-painting-in-visakhapatnam.jpg', // UPDATED: Real Painting image
...
const providers = [
    { id: 1, name: 'Electrical services', categoryId: 'app_elec', price: 80, bio: 'Certified electricians specializing in residential wiring and fault diagnostics across NCR and Delhi.' },
    { id: 11, name: 'General mechanics', categoryId: 'app_elec', price: 80, bio: 'Repair of washing machines, refrigerators, and microwaves at home.' },
    { id: 2, name: 'Plumber service', categoryId: 'plumb', price: 80, bio: 'Fast, reliable plumbing service for leaks, clogs, and installations. 10 years experience.' },
    { id: 4, name: 'Cleaning Services', categoryId: 'clean', price: 80, bio: 'Eco-friendly house cleaning services. Weekly, bi-weekly, or deep cleaning options.' },
    { id: 5, name: 'Pests control', categoryId: 'pest', price: 80, bio: 'Residential and commercial pest control. Guaranteed removal of all common pests.' },
    { id: 6, name: 'Carpenter service', categoryId: 'carp', price: 80, bio: 'Custom carpentry, repairs, and furniture assembly. Serving NCR & Delhi.' },
    { id: 3, name: 'Haircut', categoryId: 'salon_mass', price: 80, bio: 'Modern and classic haircuts for men and women. Home service available in Rohini Delhi.' },
    { id: 7, name: 'Therapy', categoryId: 'salon_mass', price: 80, bio: 'Certified therapist offering deep tissue and sports massage for pain relief.' },
    { id: 9, name: 'AC Experts', categoryId: 'ac', price: 80, bio: 'Specialized AC installation, repair, and maintenance in major cities.' },
    { id: 10, name: 'Painting service', categoryId: 'paint', price: 80, bio: 'Professional interior and exterior painting with paints.' },
];
// Mock Bookings Data with Rupee prices
const mockBookings = [
    { id: 101, providerName: 'Electrical services', serviceName: 'Appliance & Electrical Repair', date: '2025-10-25', time: '10:00 AM', status: 'Confirmed', price: 280, rating: 5 },
    { id: 102, providerName: 'Cleaning Services', serviceName: 'Deep House Cleaning', date: '2025-11-05', time: '02:00 PM', status: 'Pending', price: 280, rating: null },
    { id: 103, providerName: 'Plumber service', serviceName: 'Plumbing & Leaks', date: '2025-09-15', time: '09:00 AM', status: 'Completed', price: 280, rating: 4 },
];
...
// Authentication Functions (exposed to window in index.html, but defined here)
window.signInUser = async () => {
    ...
    try {
        // Use the function exposed from the type="module" script in index.html
...
// --- Page Render Functions (All the HTML generating functions) ---
            ...
            <div onclick="navigateTo('listings', {categoryId: '${c.id}'})" class="text-center p-4 sm:p-6 bg-white rounded-2xl shadow-lg hover:shadow-xl hover:border-blue-500 border-2 border-transparent transition-all duration-300 cursor-pointer group transform hover:-translate-y-2 professional-card fade-in">
                <div class="w-32 h-32 sm:w-40 sm:h-40 lg:w-48 lg:h-48 mx-auto mb-4 overflow-hidden rounded-2xl shadow-lg">
                    <img 
                        ...
                        alt="${c.name}" 
                        onerror="this.src='https://via.placeholder.com/192?text=${c.name.slice(0,2).toUpperCase()}'; this.onerror=null;"
                    >
                </div>
            ...
            <div class="bg-white p-6 rounded-xl shadow-lg flex flex-col md:flex-row items-start md:items-center justify-between border border-gray-100 professional-card fade-in">
                <div class="flex-grow mb-4 md:mb-0">
                    ...
                    <div class="flex items-center text-sm text-gray-600 mb-3">
                        <span class="mr-2">${getRatingStars(p.rating || 4.5)}</span>
                        <span class="font-medium">${(p.rating || 4.5).toFixed(1)} (${(p.reviews || 150)} reviews)</span>
                    </div>
...
function renderProviderProfile(provider) {
    ...
    // Mock data for reviews (since it wasn't in the main provider object)
    const mockReviews = [
    ...
    // Mock rating and review count for display
    const mockRating = 4.5;
    const mockReviewsCount = 150;
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div class="lg:col-span-2 space-y-8">
                ...
                <div class="flex items-center space-x-6">
                    ...
                    <div>
                        ...
                        <div class="flex items-center mt-2">
                            <span class="text-yellow-400 text-lg mr-1">${getRatingStars(mockRating)}</span>
                            <span class="text-gray-700 font-medium">(${mockReviewsCount} reviews)</span>
            ...
            <div class="bg-white p-6 rounded-xl shadow-lg professional-card">
                <h3 class="text-2xl font-bold mb-4 text-gray-800 border-b pb-2">Customer Reviews (${mockReviewsCount})</h3>
                <div class="space-y-4">
                    ${mockReviews.map(r => `
                ...
                <button class="mt-4 text-blue-600 hover:underline font-medium">Read, Read All ${mockReviewsCount} Reviews</button>
            </div>
        </div>
    ...
    switch (step) {
        case 1:
            ...
            <select id="booking-time" required class="w-full p-3 border border-gray-300 rounded-lg mb-6 focus:ring-blue-500 focus:border-blue-500">
                ...
                <option value="9:00 AM" ${appState.currentBooking.time === '9:00 AM' ? 'selected' : ''}>9:00 AM - 11:00 AM</option>
                <option value="1:00 PM" ${appState.currentBooking.time === '1:00 PM' ? 'selected' : ''}>1:00 PM - 3:00 PM</option>
                <option value="3:00 PM" ${appState.currentBooking.time === '3:00 PM' ? 'selected' : ''}>3:00 PM - 5:00 PM</option>
            </select>
        ...
        case 3:
            ...
            const problemCharge = 200; // Mocking a variable problem charge
            const price = visitingCharge + problemCharge;
            const taxes = price * 0.18; // Mocking 18% GST/Taxes
        ...
        case 4:
            // Final step: Confirming and adding to mockBookings
            const newBooking = {
                ...
                status: 'Confirmed',
                price: 280, // Using mock price for simplicity
                rating: null
            };
...
// Expose the core functions globally for the HTML file to access
